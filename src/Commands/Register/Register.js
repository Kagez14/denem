...
if(![env.Staff.register_staff, env.Staff.all_commands].some(role => message.member.roles.cache.get(role)) && !message.member.hasPermission('ADMINISTRATOR')) return message.channel.send(yanlisEmbed.setDescription(`Bu komutu sadece (<@&${env.Staff.register_staff}>, <@&${env.Staff.all_commands}>) yetkisine sahip olanlar kullanabilir.`).setFooter(`Developed By Kagez.`)).then(m => m.delete({timeout:6500}))

const member = message.guild.member(message.mentions.members.first() || message.guild.members.cache.get(args[0]));
if(!member) return message.channel.send(yanlisEmbed.setDescription(`Bir kullanıcı belirtmelisiniz.`).setFooter(`Developed By Kagez.`)).then(m => m.delete({timeout:6500}))
if(!args[1]) return message.channel.send(yanlisEmbed.setDescription(`Bir isim belirtmelisin.`).setFooter(`Developed By Kagez.`)).then(m => m.delete({timeout:6500}))
if(!args[2]) return message.channel.send(yanlisEmbed.setDescription(`Bir yaş belirtmelisin.`).setFooter(`Developed By Kagez.`)).then(m => m.delete({timeout:6500}))
let Name = args[1].charAt(0).toUpperCase() + args[1].slice(1).toLowerCase();
let Age = Number(args[2])
if(member.id === message.author.id) return message.channel.send(yanlisEmbed.setDescription(`Kendini kayıt edemezsin.`).setFooter(`Developed By Kagez.`)).then(m => m.delete({timeout:6500}))
if(member.id === member.manageable) return message.channel.send(yanlisEmbed.setDescription(`Belirtilen kullanıcı üzerinde komut geçersiz.`).setFooter(`Developed By Kagez.`)).then(m => m.delete({timeout:6500}))
if(member.roles.highest.position >= message.member.roles.highest.position) return message.channel.send(yanlisEmbed.setDescription(`Kendini kayıt edemezsin.`).setFooter(`Developed By Kagez.`)).then(m => m.delete({timeout:6500}))
...
.setFooter(`Developed By Kagez.`)).then(m => m.delete({timeout: 7500}))
...
if(registerCheck.size === null) return message.channel.send(new MessageEmbed().setDescription(`${message.author} adlı yetkili ${member} kullanıcısı için bir cinsiyet belirtmediği için işlem iptal edildi.`).setColor(env.Colors.anlik).setFooter(`Developed By Kagez.`)).then(x => x.delete({timeout: 6500}));

let manCheck = registerCheck.first() 
if(manCheck.content.toLowerCase().includes(`erkek`)) {
message.channel.send(new MessageEmbed().setDescription(`${member} kullanıcısının ismi "${ServerName}" olarak değiştirildi \nKullanıcıya ${message.author} tarafından <@&${env.Roles.man_role}> verildi.`).setAuthor(member.user.tag, member.user.avatarURL({dynamic:true})).setColor(env.Colors.blue).setFooter(`Developed By Kagez.`))
member.setNickname(`${ServerName}`)
member.roles.add(env.Roles.man_role)
member.roles.remove(env.Roles.unregister_role)

client.channels.cache.get(env.Channels.register_channel).send(new MessageEmbed().setTitle(`Bir Üye Kayıt Edildi !`).setDescription(`\`•\` Kullanıcı: ${member} | (${member.id})\n\`•\` Yetkili: ${message.author} | (${message.author.id})\n\n\`•\` Yeni İsim: \`${ServerName}\`\n\`•\` Kullanıcının Cinsiyeti: \`Erkek\`\n\`•\` Verilen Rol: <@&${env.Roles.man_role}>\n\`•\` Kayıt Saati: \`${kayıtsaat}\`\n\`•\` Kayıt Tarihi: \`${moment(Date.now()).format("DD MMMM YYYY")}\``).setThumbnail(member.user.avatarURL({dynamic:true})).setColor(env.Colors.blue).setFooter(`Developed By Kagez.`))
...
message.channel.send(new MessageEmbed().setDescription(`${member} kullanıcısının ismi "${ServerName}" olarak değiştirildi \nKullanıcıya ${message.author} tarafından <@&${env.Roles.woman_role}> verildi.`).setAuthor(member.user.tag, member.user.avatarURL({dynamic:true})).setColor(env.Colors.pink).setFooter(`Developed By Kagez.`))
member.setNickname(`${ServerName}`)
member.roles.add(env.Roles.woman_role)
member.roles.remove(env.Roles.unregister_role)

client.channels.cache.get(env.Channels.register_channel).send(new MessageEmbed().setTitle(`Bir Üye Kayıt Edildi !`).setDescription(`\`•\` Kullanıcı: ${member} | (${member.id})\n\`•\` Yetkili: ${message.author} | (${message.author.id})\n\n\`•\` Yeni İsim: \`${ServerName}\`\n\`•\` Kullanıcının Cinsiyeti: \`Kadın\`\n\`•\` Verilen Rol: <@&${env.Roles.woman_role}>\n\`•\` Kayıt Saati: \`${kayıtsaat}\`\n\`•\` Kayıt Tarihi: \`${moment(Date.now()).format("DD MMMM YYYY")}\``).setThumbnail(member.user.avatarURL({dynamic:true})).setColor(env.Colors.pink).setFooter(`Developed By Kagez.`))
...
.setFooter(`Developed By Kagez`)).then(m => m.delete({timeout:6500}))}
